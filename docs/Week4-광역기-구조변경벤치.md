...
> 상태: ✅ **완료** — ① 광역기 ✅ · ② 구조 변경 A/B ✅ · ③ 플레이어 사망 ✅ · ④ 넉백 ✅ · ⑤ 경직 ✅ · 캐릭터/카메라/콤보 → [`Week4-캐릭터-콤보-루트모션.md`](Week4-캐릭터-콤보-루트모션.md)

**기간**: 2026-07-17 ~
**목표**: 광역기(범위 수천 동시 피격) · **"Enableable이 진짜 빠른가" 수치 증명** · 양방향 전투의 완성도(사망·넉백·경직)
...
## ⑤ 경직 — **시스템 간 계약**이 처음 생긴다

넉백이 손맛의 절반이라면 경직이 나머지다. 그리고 이번 단계엔 **새 개념**이 하나 있다.

### 먼저 용어

| | 뜻 | 이 프로젝트에선 |
|---|---|---|
| **경직 (hitstun)** | 맞은 **놈이** 잠깐 못 움직임 | 적 1만 마리 각자 상태 → **ECS** (이번 작업) |
| 히트스톱 (hit freeze) | **화면 전체**가 순간 멈춤 | `Time.timeScale` 조작 → GO 쪽, W6 폴리시로 |

### 새 개념 — 시스템이 처음으로 **엮인다**

지금까지 시스템들은 **서로 남남**이었다. 각자 자기 것만 읽고 썼다.

```
DamageApplySystem ──(맞으면 Stun 세팅)──▶ Stun 컴포넌트 ◀──(움직일지 물어봄)── MovementSystem
     쓰는 쪽                                                        읽는 쪽
```

`MovementSystem`이 이제 **"경직 중이면 안 움직인다"를 알아야** 한다. 두 시스템이 `Stun`이라는 데이터를 통해 처음 계약을 맺는다.

### 구성 (`EnemyAttack.Timer`와 판박이)

```csharp
public struct Stun : IComponentData { public float Remaining; }   // Baker가 0으로 시작
```
```csharp
// DamageApplyJob — 맞으면 세팅 (쓰는 쪽)
if (total > 0) { health.Value -= total; stun.Remaining = maxStun; }

// MoveJob — 움직이기 전에 물어봄 (읽는 쪽)
if (stun.Remaining > 0f) { stun.Remaining -= DeltaTime; return; }   // 경직 중 → 안 움직임
```

`StunDuration`을 체인으로 흘려보냈다(`AttackRequest → DamageEvent → 적용`) — `KnockbackScale`과 똑같은 패턴. `maxStun`도 **최대 데미지 건 기준**: 제일 센 놈이 방향·세기·경직을 다 정한다.

### 🔴 함정 — 가드 순서 (조용히 틀린, **데모도 통과하는** 버전)

처음엔 **거리 체크를 경직 체크보다 앞**에 뒀다:

```csharp
if (distance < StopDistance) return;         // ← StopDistance 안이면 여기서 나감
if (stun.Remaining > 0) { stun.Remaining -= dt; return; }   // ← 도달을 못 함
```

적들은 플레이어에 붙어 **StopDistance 안에 뭉쳐** 있다 → 그 적들은 거리 체크에서 먼저 `return`해서 **경직 타이머가 안 깎이고 얼어붙는다.**

**무서운 건 넉백이 이걸 가려준다는 점.** 넉백이 적을 StopDistance **밖으로** 밀어내면 다음 프레임엔 거리 체크를 통과해 경직이 제대로 돈다. 그래서 광역기 데모는 "밀려났다 잠깐 멈췄다 온다"로 **정확히 보인다** — 로직은 틀렸는데.

```csharp
// 고침: 경직을 거리 체크 앞으로
if (stun.Remaining > 0) { stun.Remaining -= dt; return; }   // 위치와 무관하게 벽시계처럼
if (distance < StopDistance) return;
```

> **계약의 미묘함**: 무엇을 공유하나(`Stun`)뿐 아니라 **"언제 확인하나"** (거리 체크 전)까지 맞아야 한다. 이 프로젝트에서 반복된 **"초록불인데 조용히 틀린"** 부류인데, 이번엔 **런타임 데모까지 통과**하는 버전이었다.

### 🔬 검증 — 넉백 0으로 껐다 (경직만 격리)

넉백이 가려주는 걸 막으려고 **`KnockbackScale=0`, `StunDuration=15`(크게)** 로 쐈다.

```
발사 → 경직중 = 10000 (전원)
t=190.69  남은 12.69
t=192.85  남은 10.52   (−2.17 / 2.16초 경과)
...
t=202.46  남은  0.91
t=204.43  경직중 = 0   (전원 동시 해제)
```

- **① 벽시계로 정확** — 남은 시간이 실시간과 **1:1**(2초에 2 감소). `DeltaTime` 감쇠 정확. 시작값 12.69도 `15 − 발사지연 2.3 ≈ 12.7`로 일치.
- **② 전원 10000 경직** — **순서 수정의 증거.** 붙어있는 적(StopDistance 안)까지 전부 깎였다. 옛 순서였다면 10000이 안 나온다. 넉백을 껐으므로 **넉백이 가려주는 게 아님도 확인.**
- **③ 동시 해제** — 같은 프레임에 맞았으니 같은 프레임에 풀린다.

### 넉백 + 경직 = 손맛

넉백만이면 툭 밀렸다 **~2초 만에 걸어온다**(④ 검증에서 관찰). 경직이 그 앞에 **정지 구간**을 넣어 "때렸다"가 눈에 읽힌다. 광역기(넉백 5 + 경직 0.5)를 쏘면 **밀려남 → 0.5초 얼어붙음 → 재접근**.
...
- **시스템 간 계약** — 경직으로 두 시스템이 처음 엮였다(`DamageApply` 쓰기 → `Move` 읽기). 계약은 **무엇을 공유하나뿐 아니라 "언제 확인하나"** 까지 맞아야 한다 — 경직 체크를 거리 체크 앞/뒤 어디 두느냐로 의미가 갈렸다.
...
- **격리해야 진짜 검증** — 경직도 넉백을 0으로 꺼서 "넉백이 가려주는 것"과 분리했다. **관측값이 다른 원인으로도 설명되면 그 실험은 아무것도 증명하지 못한다.**
...
- [x] **경직** — `Stun` 컴포넌트 · `MovementSystem`이 존중 (2026-07-17). **넉백+경직으로 손맛 완성**
- [ ] **히트스톱** — 경직과 다르다. 화면 전체 순간 멈춤(`Time.timeScale`) → GO 쪽 폴리시, W6로
- [ ] 오버숄더 카메라 · 콤보 (W3에서 이월)
